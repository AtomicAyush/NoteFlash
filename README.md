...
- **Sharing a deck:** **Share Deck** (in a deck's ⋯ menu, or by holding a deck in the list) sends the deck as a single web page file. To add one you've been sent, share it to NoteFlash (or "Open With" NoteFlash) and confirm. Anyone can open it — on a phone, a computer, or in email — and study it there or print it; nothing is uploaded and no account is needed. Someone who has NoteFlash can open the same file in the app, which adds the cards exactly as written, exam priorities and notes included, with their own study progress.
