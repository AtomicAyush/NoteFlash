...
- **Saved sections:** `SectionCache` writes finished sections to disk as well as memory, so a job that stops halfway resumes from where it stopped instead of spending the usage limit again. They're kept for three days, and keyed by the job that wrote them (a task-local set while the job runs), so they're only ever reused by that same job.
- **Not grinding in the background:** a paused job is only started again while the app is open or while iOS is giving the app background time. Without that, a job gets about half a minute — less than a section takes — so it would be cut off over and over and lose the work each time.
- **Picking work up:** `restoreSavedJobs` runs at launch (not when a window appears — iOS also starts the app with no window), when the app becomes active, and during background runs. Jobs that can't succeed on a retry (notes NoteFlash can't read, a blocked or deleted deck, a sign-in that expired) aren't retried; the rest are, up to four failures before waiting for **Retry**. Being stopped by iOS isn't a failure and doesn't count.
...
- **Usage limits:** iOS limits how much Apple Intelligence work an app can do in a stretch, and long notes can reach that limit even while NoteFlash is open. `ModelLimits` handles it:
  ...
  - **No repeated work:** finished sections are kept (`SectionCache`) for the job that wrote them, so a resumed or retried job skips them while a new deck — or a deck being rewritten — asks the model again rather than handing back cards written for something else. Optional extra passes are skipped for 15 minutes after a limit.
...
- **Content check:** the system's own content check sometimes fails under load (`SensitiveContentAnalysisML`). That's treated as a passing problem, so the section is retried as plain text instead of failing the deck.
